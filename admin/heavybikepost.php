`<html>
 ...
 {  
      $output = '';  
      $sql = "SELECT heavybikes FROM heavybikes";  
      $result = mysqli_query($con, $sql);  
      while($row = mysqli_fetch_array($result))  
      {  
           $output .= '<option value="'.$row["heavybikes"].'">'.$row["heavybikes"].'</option>';  
      }  
      return $output;  
 }  
...
<!-- Document Wrapper
	...
	<form method="post" action="heavybikepost.php" enctype="multipart/form-data">
					...
					<div class=" nobottommargin clearfix">
							...
							<div class="col-md-3 bottommargin-sm">
									<div class="white-section">
										...
										while($row=mysqli_fetch_array($rows)){
											?>
											<option value="<?php echo $car = $row['bmakers']; ?>"><?php echo $car = $row['bmakers'];?></option>
										<?php } ?>
										</select>
									</div>
							</div>
						...
						<div class="row">	
							...
							<div class="col-md-4 topmargin-sm">
									<label for="template-contactform-message">Speedometer <small>*</small></label>
									<input type="text" class="required sm-form-control" id="" name="speedometer" rows="1" cols="30" value="0">
								</div>
								
							<div class="col-md-6 topmargin-sm">
									<label for="template-contactform-message">Top Speed (Tested) <small>*</small></label>
									<input type="text" class="required sm-form-control" id="" name="topspeed" rows="1" cols="30" value="0">
								</div>
...
if(isset($_POST['submit'])){ 	
	...
	if ($imaget=="image/jpeg"or$imaget=="image/png"or$imaget=="image/gif" ){
		...
		$data=mysqli_query($con,"INSERT INTO heavybikeposts (author,created,updated,company,model,pmaxl,pmaxt,pminl,pmint,year,img1,img2,img3,img1thumb,img2thumb,img3thumb,sdesp,desp,engine,millage,type,start,pc,speedometer,topspeed,gearbox,edisp) VALUES ('$author','$date','$date','$company','$model','$pmaxl','$pmaxt','$pminl','$pmint','$year','$imagen','$imagen1','$imagen2','$imagenthumb','$imagen1thumb','$imagen2thumb','$sdesp','$desp','$engine','$millage','$type','$start','$pc','$speedometer','$topspeed','$gearbox','$edisp')")or die( mysql_error() );
