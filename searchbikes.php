...
<meta property="og:url"  content="https://www.riderspk.com/"/>
	...
	<title><?php if ( !empty ($company) ) echo $company ; ?> Bikes Details Search</title>
	...
	<!-- Content
		...
		<section id="">
			...
			<div class="content-wrap" style="padding:20px 0;">
				...
				<div class="col-md-3">
				<form method="post" action="search_bikes" enctype="multipart/form-data">
...
if(isset($_POST['submit']) ){ 	
	...
	$order = $_POST['order'];
		...
		$sql = "SELECT * FROM bikeposts WHERE  company REGEXP ('$company') AND year REGEXP ('$year') AND gearbox REGEXP ('$gearbox') AND start REGEXP ('$startid') AND pmaxl <= ('$pmaxl') AND edisp >= ('$edisp') AND millage >= ('$millage') AND pc >= ('$pc') ORDER BY $sort $order ";
		$result = $con->query($sql)or die($con->error);
		echo $tresults=mysqli_num_rows($result);
		$sql = "SELECT * FROM bikeposts WHERE  company REGEXP ('$company') AND year REGEXP ('$year') AND gearbox REGEXP ('$gearbox') AND start REGEXP ('$startid') AND pmaxl <= ('$pmaxl') AND edisp >= ('$edisp') AND millage >= ('$millage') AND pc >= ('$pc') ORDER BY $sort $order LIMIT 8 ";
		$result = $con->query($sql)or die($con->error);
	...
	<?php 
		...
		?>
							<div class="entry clearfix nobottommargin notopmargin">
								...
								<ul class="entry-meta clearfix"  style="float: left;font-size: 13px;line-height: 14px;margin: 0 10px 15px 0;font-family: 'Crete Round', serif;font-style: italic;">
									...
									<i ="classicon-filter"></i> <?php echo $row['millage'] ?> &nbsp &nbsp
								...
								<div class="entry-content">
									...
									<a href="bike-<?php  echo $row['post_id']  ?>-<?php $mod=$row['model']; echo $modstring = preg_replace("/[\s-]/", "_", $mod); ?>_<?php echo $row['year'] ?>"class="more-link">View</a>
								</div>
							</div>
		...
		$sql = "SELECT * FROM bikeposts WHERE  company REGEXP ('$company') ORDER BY company desc";
		$result = $con->query($sql)or die($con->error);
		echo $tresults=mysqli_num_rows($result);
		
		$sql = "SELECT * FROM bikeposts WHERE  company REGEXP ('$company') ORDER BY company desc LIMIT 8 ";
		$result = $con->query($sql)or die($con->error);
